...
-- Compatibility shim for FS25: some engine versions changed wheel API.
-- If vehicle.spec_wheels is missing but vehicle:getWheels() exists, populate spec_wheels.wheels
local function ensureSpecWheels(vehicle)
    if vehicle == nil then return end
    if vehicle.spec_wheels == nil and type(vehicle.getWheels) == 'function' then
        local ok, wheels = pcall(vehicle.getWheels, vehicle)
        if ok and wheels ~= nil then
            -- wrap into legacy structure expected by FS22 scripts
            vehicle.spec_wheels = vehicle.spec_wheels or {}
            vehicle.spec_wheels.wheels = wheels
        end
    end
end

-- Monkeypatch Vehicle update hooks where necessary by overriding onUpdate to ensure spec is present.
local origOnUpdate = nil
if Vehicle ~= nil and type(Vehicle.onUpdate) == 'function' then
    origOnUpdate = Vehicle.onUpdate
    Vehicle.onUpdate = function(self, dt, ... )
        ensureSpecWheels(self)
        return origOnUpdate(self, dt, ... )
    end
end
